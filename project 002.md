...
- [ ] **P002-G6-S01 — Source resolver.** Với job R2, tải object theo `storageKey`; với job legacy, fallback `filePath`.
- [ ] **P002-G6-S02 — Download atomically.** Stream vào file `.part`, kiểm tra đủ byte rồi rename; không xử lý file tải dở.
- [ ] **P002-G6-S03 — Validate lại PDF.** Kiểm tra magic bytes/PDF parse sau download; không tin MIME từ browser.
- [ ] **P002-G6-S04 — Disk admission.** Trước download kiểm tra `sourceSize` và budget disk; backend vẫn chỉ giữ tối đa số source file theo cấu hình.
- [ ] **P002-G6-S05 — Restart recovery.** Lease hết hạn trả job về pending; worker mới xóa `.part` cũ và tải lại từ R2.
- [ ] **P002-G6-S06 — Không còn permanent `FILE_MISSING` vì Render.** Mất file local chỉ kích hoạt redownload; chỉ báo source missing nếu R2 HEAD thực sự 404.
- [ ] **P002-G6-S07 — Resume chunk.** Tận dụng TranslationChunk P001; redownload/retry không dịch lại chunk đã lưu.
- [ ] **P002-G6-S08 — Cleanup local.** Xóa file tạm trong `finally` sau success/failure/cancel; startup orphan scan vẫn hoạt động.
- [ ] **P002-G6-S09 — R2 timeout/error taxonomy.** Phân biệt auth/config, object missing, rate limit, timeout và unavailable; retry đúng loại, không tăng Gemini circuit breaker.
- [ ] **P002-G6-S10 — Streaming/memory test.** File 30 MB và file gần max không tạo Buffer copy toàn file ngoài nhu cầu thư viện PDF.
...
- [ ] **P002-G7-S01 — Centralize source cleanup.** Mọi đường completed/failed/cancel/delete gọi cùng service, có retry và idempotency.
- [ ] **P002-G7-S02 — Completed.** Chỉ sau khi kết quả/chunks đã commit an toàn mới DELETE source R2; lỗi DELETE không đổi completed thành failed nhưng phải xếp cleanup retry.
- [ ] **P002-G7-S03 — Retryable failure.** Giữ object R2 trong thời gian retry; không phụ thuộc file local.
- [ ] **P002-G7-S04 — Permanent failure.** Xóa object theo policy sau khi lưu error rõ; cân nhắc giữ ngắn hạn nếu cần người dùng retry thủ công trong retention window.
- [ ] **P002-G7-S05 — Cancel/delete.** Dừng pipeline, xóa local, chunks theo semantics hiện tại, xóa object R2 và Job/Batch đúng thứ tự phục hồi được.
- [ ] **P002-G7-S06 — Uploading orphan.** Job chuẩn bị nhưng chưa PUT phải hết hạn; object PUT nhưng chưa confirm được reconciler phát hiện hoặc lifecycle xóa.
- [ ] **P002-G7-S07 — Cleanup retry queue.** Persist số lần/thời điểm thử xóa R2; restart không làm quên object cần xóa.
- [ ] **P002-G7-S08 — Batch cleanup.** Batch chỉ terminal khi từng item đã terminal/removed; xóa folder xử lý cả R2 sources.
- [ ] **P002-G7-S09 — Lifecycle verification.** Xác nhận rule `incoming/` 3 ngày tồn tại ở production và ghi cảnh báo nếu bị tắt.
- [ ] **P002-G7-S10 — Reconciliation report.** Script read-only báo số job thiếu object, object tham chiếu, object orphan và tổng byte; không tự xóa trong dry-run.
