...
| Mã kế hoạch | P003 |
| Ngày lập | 15-07-2026 |
| Trạng thái tổng | V2 đã hoàn tất local implementation/regression; B4 và full-corpus v1 chỉ là bằng chứng lịch sử, còn benchmark và review chuyên môn B5 mới là cổng quyết định rollout |
...
| G1 | Benchmark minimal/medium/high và pipeline nhiều lượt | Đang thực hiện (9/12; B4 bị loại làm ứng viên production) | Có báo cáo so sánh mù, chốt B5 bằng bằng chứng |
...
| G9 | Rollout production và theo dõi | Đang thực hiện (2/11) | Canary đạt, bật quality an toàn, rollback đã kiểm chứng |
...
| B5 | 2 trang | 1.0 | high | Document context → Translate → Audit/coverage → Revise → Verify/coverage; repair có điều kiện |
...
- [x] **P003-G1-S06 — Chạy B4.** Lưu riêng output từng stage và kết quả PASS/FAIL/repair. Đây là bằng chứng v1 lịch sử, không còn là ứng viên production.
...
- [ ] **P003-G1-S11 — Chốt bằng chứng.** B4 đã bị loại sau review chủ dự án vì không có ngữ cảnh toàn tài liệu và tự-audit thưa; chỉ giữ B5 nếu giảm lỗi critical, không tăng omission và qua review chuyên môn.
- [ ] **P003-G1-S12 — Chạy B5 và chấm mù lại.** Với mỗi PDF, sinh một passport từ toàn tài liệu rồi benchmark đúng page range cố định; kiểm review bundle không chỉ theo self-flag và so B5 với B3/B4 trước khi cân nhắc rollout.
...
- [ ] **P003-G8-S11 — Hiệu năng.** Phần live corpus đã đạt: đủ 370 trang/191 chunk, 0 task failed, trung bình 78.342 ms/chunk, p95 109.012 ms, tối đa 132.563 ms/chunk; single call tối đa 42.123 ms dưới timeout 180 giây. Runner benchmark dùng concurrency 4 và resume 43 artifact; báo cáo ngoại suy production target concurrency 2 là 124,8 phút model wall-time. Còn phải xác nhận timeout/lease trên Render ở canary trước khi đánh dấu hoàn thành.
...
- [ ] Benchmark B5 toàn tài liệu + coverage và review chuyên môn độc lập đã đạt.
...
- Full-corpus B4 live đã hoàn tất trước lượt tổng hợp hiện tại; không còn runner Node. Checkpoint có `completedAt`, 191/191 processed, 0 failed; stderr rỗng. Đây là corpus v1 lịch sử, không phải xác nhận chất lượng để rollout v2.
...
1. Chạy B5 trên benchmark cố định, bắt đầu bằng một PDF đại diện bằng `node med-translator-backend/scripts/benchmark-project-003.js --variant B5 --file "…pdf" --start-page N`; thêm `--dry-run` khi chỉ cần kiểm plan. Sau đó mới chạy batch B5 có checkpoint. Không dùng lại artifact B4 để skip B5.
2. Tạo review bundle v2 theo mẫu đa dạng (không chỉ chunk self-flag) và chấm mù trước khi mở answer key. Blind review A–D cũ vẫn được giữ nguyên và chưa cần chấm tiếp.
3. Hoàn tất G8-S12 bằng số liệu RAM/disk/Mongo thực ở canary; báo cáo local hiện chỉ đo RSS khi split PDF và BSON payload trước compression/index overhead.
4. Deploy backend additive với mode `legacy`, smoke API/schema/SSE và một job legacy; sau đó deploy frontend tương thích ngược (G9-S02/S03). Đây là thay đổi production, cần chủ dự án cho phép trước khi thực hiện.
5. Khi chủ dự án cho phép: canary quality B5 một PDF ngắn, rồi một PDF dài; theo dõi stage latency, lease heartbeat, 429, artifact và cleanup (G9-S04).
...
8. Đóng dự án G9-S11 khi chủ dự án đã duyệt chất lượng và các cổng B5/canary đều đạt.
