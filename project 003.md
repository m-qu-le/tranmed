...
- [x] **P003-G8-S10 — Cổng chấp nhận phương án.** Chủ dự án đã duyệt review-bundle, chốt flow B4 và không yêu cầu blind review. Việc bật mặc định vẫn chỉ diễn ra sau canary/batch production, không chỉ dựa trên self-judge của Flash-Lite.
...
- [ ] Canary B4-v2 xác nhận context/coverage hoạt động trên PDF ngắn và dài mà không làm tăng omission hoặc lỗi nghiêm trọng.
...
- Full-corpus B4 live đã hoàn tất trước lượt tổng hợp hiện tại; không còn runner Node. Checkpoint có `completedAt`, 191/191 processed, 0 failed; stderr rỗng. Chủ dự án đã chọn flow B4; context/coverage v2 sẽ được xác nhận ở canary thay vì tạo thêm vòng blind review.
...
3. Deploy backend additive với mode `legacy`, smoke API/schema/SSE và một job legacy; sau đó deploy frontend tương thích ngược (G9-S02/S03).
4. Canary quality B4-v2 một PDF ngắn, rồi một PDF dài; theo dõi context stage, coverage, latency, lease heartbeat, 429, artifact và cleanup (G9-S04).
