...
- Ngay dưới input thường có vùng `⚡ Hàng đợi ưu tiên`; drop hoặc chọn PDF tại đây tự upload batch `priority: true` vào group hệ thống `Ưu tiên`.
...
- Batch close-safe có thể Ẩn/Ẩn tất cả bằng danh sách `batchId` trong local storage; đây không phải lệnh xóa backend và batch chưa an toàn không được ẩn.
- Khi một upload đang chạy, priority batch mới được xếp local và tự khởi động ngay sau upload hiện tại; upload R2 không bị preempt.

## Jobs, SSE và chất lượng

- Initial load/reconnect lấy `/status`, `/jobs`, `/jobs/stats` và `/upload-batches`; SSE chỉ là kênh cập nhật, không phải nguồn trạng thái duy nhất.
- Job `priority=1` được group thành `📌 Ưu tiên`, luôn render trước group folder thường và tự biến mất khi không còn job priority.
- Dashboard dùng `/jobs/stats` cho tổng toàn collection; tải thêm trang lịch sử không được làm đổi số tổng.
...
- Header chi tiết P004 do backend dựng một lần; Preview render, Copy dùng đúng chuỗi result và Download dùng cùng nội dung cảnh báo.
- Legacy job không có quality fields vẫn hoạt động.

## Kết quả

- Job card lazy-fetch result để preview/copy; list summary không chứa Markdown đầy đủ hoặc report P004.
