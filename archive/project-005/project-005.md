...
| Mã kế hoạch | P005 |
| Ngày lập | 17-07-2026 |
| Trạng thái tổng | **HOÀN THÀNH — production giữ concurrency 2; canary hai lane, output và cleanup đạt; peak RAM trực tiếp được owner waiver** |
...
- [x] **P005-G7-S01 — Deploy code với concurrency 1.** Backend trước; xác nhận health/readiness/status/jobs/stats đạt.
- [x] **P005-G7-S02 — Deploy frontend.** Xác nhận bundle production có card, `/jobs/stats` và nút Ẩn tất cả.
- [x] **P005-G7-S03 — Kiểm hàng đợi sạch.** Trước canary, ghi pending/processing và không chen canary vào batch người dùng đang chạy.
- [x] **P005-G7-S04 — Chuẩn bị fixture.** Hai PDF tổng hợp một trang, 1.551 và 1.550 byte; không dùng tài liệu người dùng.
- [x] **P005-G7-S05 — Bật concurrency 2.** Render env đã được chủ dự án đặt; `/status.worker.concurrency` trả 2.
- [x] **P005-G7-S06 — Chạy canary.** Hai fixture cùng đạt `processing`; quan sát `activeJobs=2`, `activeSourceBytes=3.101`.
- [x] **P005-G7-S07 — Cổng tài nguyên.** Active source thấp hơn budget 10 MiB, không crash/restart/OOM quan sát được; phép đọc trực tiếp peak RAM Render <80% không có phiên dashboard và được owner waiver khi yêu cầu đóng P005.
- [x] **P005-G7-S08 — Cổng quota/lifecycle.** Cả hai completed ngay attempt đầu; không retry/429/5xx/lease expiry; cleanup/upload backlog trở về 0.
- [x] **P005-G7-S09 — Kiểm output.** Preview/result và download khớp riêng từng job, hai output có độ dài/hash khác nhau; không trộn chunk/job.
- [x] **P005-G7-S10 — Dọn canary.** Xóa batch/job fixture; queue, source active và backlog đều trở về 0; fixture/log local được xóa khi đóng hồ sơ.
- [x] **P005-G7-S11 — Quyết định giữ hoặc rollback.** Giữ production concurrency 2 vì các cổng quan sát được đạt; rollback về 1 vẫn là thao tác một biến môi trường nếu có dấu hiệu RAM bất thường.
...
- [x] **P005-G8-S01 — Cập nhật bảng tiến độ.** Mọi giai đoạn và waiver RAM được ghi theo bằng chứng thật.
- [x] **P005-G8-S02 — Ghi commit/deploy.** Ghi mốc `88d081d`, `036af2c`, `9f16280`, production concurrency 2 và trạng thái endpoint; không lưu secret.
- [x] **P005-G8-S03 — Ghi kết quả canary.** Ghi hai fixture, peak active/source, attempt, output và cleanup; không suy diễn peak RAM chưa đo.
- [x] **P005-G8-S04 — Chốt known gaps.** Không theo dõi 24 giờ, không thử file lớn và không đọc trực tiếp peak RAM Render.
- [x] **P005-G8-S05 — Xác nhận không còn runner.** PID canary đã kết thúc thành công; script, fixture và log local được xóa; queue production sạch.
- [x] **P005-G8-S06 — Đóng checklist.** G0–G8 đạt, riêng peak RAM có owner waiver; hồ sơ được chuyển vào archive.
...
| G1 | Backend job stats | Hoàn thành | Tổng MongoDB đúng, độc lập pagination |
| G2 | Dashboard realtime | Hoàn thành | Tải thêm lịch sử không đổi số |
| G3 | Ẩn batch bền vững | Hoàn thành | Safe-only, F5/SSE không phục hồi |
| G4 | Lõi worker pool | Hoàn thành | Pool tối đa 2, default 1 |
| G5 | Admission/FIFO/lifecycle | Hoàn thành | Tổng ≤10 MiB, cancel/lease/retry an toàn |
| G6 | Regression và tài liệu | Hoàn thành | Backend/frontend/diff đạt |
| G7 | Deploy và canary | Hoàn thành với waiver peak RAM | Hai job thật đồng thời; output/cleanup đạt |
| G8 | Bàn giao và đóng | Hoàn thành | Bằng chứng và giới hạn được ghi đầy đủ |
...
| 18-07-2026 | D010 | Không đổi default code sang 2 để né thiếu quyền dashboard; production giữ 1 cho tới khi đổi Render Environment có kiểm soát | Đã chốt |
| 18-07-2026 | D011 | Sau canary hai job đạt ngay attempt đầu và cleanup sạch, giữ production `TRANSLATION_WORKER_CONCURRENCY=2` | Đã chốt |
| 18-07-2026 | D012 | Browser runtime không có phiên Render nên không đọc được peak RAM trực tiếp; chủ dự án yêu cầu hoàn thiện P005 và chấp nhận đóng với waiver rõ ràng cho phép đo này | Owner waiver |
...
| 18-07-2026 | P005-G7-S01-S03 | Commit `88d081d` và commit cleanup P003 `036af2c` được push atomically lên `main` và feature; production health/readiness đạt, Mongo/R2 available, worker concurrency 1, active 0, source bytes 0, cleanup/upload backlog 0, stats 0/0/0/0 | Backend additive đã deploy an toàn với lane đơn; queue sạch trước canary |
| 18-07-2026 | P005-G7-S02 | Vercel production trả 200 với asset `index-BajcwAxq.js`/`index-DBkC36CS.css`; bundle có `File đã xong`, `/jobs/stats` và `Ẩn tất cả` | Frontend P005 đã deploy; kiểm bằng HTTP bundle, chưa thay thế visual browser QA |
| 18-07-2026 | P005-G7-BLOCKER | Browser runtime không có browser/tab và máy không có Render CLI/API credential; không thể đổi `TRANSLATION_WORKER_CONCURRENCY` trên dashboard | Dừng đúng tại S05; không tự đổi default code hoặc chạy canary giả ở concurrency 1 |
| 18-07-2026 | P005-G7-S04-S09 | Run `77373844-8e11-40ba-a901-75e5216852eb`: hai PDF tổng hợp 1.551 + 1.550 byte; production trả concurrency 2; quan sát peak `activeJobs=2`, `activeSourceBytes=3.101`; cả hai completed attempt 1; output dài 130/132 byte với SHA-256 khác nhau và result/download khớp | Canary hai lane, admission, lifecycle và phân tách output đạt; không có retry/failure |
| 18-07-2026 | P005-G7-S10-S11 | Sau delete: worker active 0/source 0, cleanup backlog 0, upload backlog 0, stats 0/0/0/0; HTTP hậu kiểm tiếp tục trả concurrency 2 và storage available | Dọn production sạch và quyết định giữ hai lane |
| 18-07-2026 | P005-G8 | PID runner `11432` đã kết thúc; `complete.json` success, không có failure marker, stderr lượt chạy thành công rỗng; browser list vẫn rỗng nên không đọc được biểu đồ RAM | Runner/script/fixture/log local được xóa; ghi known gap và owner waiver, không gọi source-size là phép đo RAM |

## 17. Tiêu chí đóng dự án

- [x] Mọi bước G0–G8 hoàn thành hoặc có waiver rõ ràng từ chủ dự án.
- [x] Dashboard đúng trên toàn bộ MongoDB và độc lập hoàn toàn với “Tải thêm lịch sử”.
- [x] UX Ẩn/Ẩn tất cả safe-only, bền vững qua F5/SSE và không mutation server.
- [x] Worker concurrency 1 tương thích cũ; concurrency 2 tuân thủ tổng 10 MiB và FIFO.
- [x] Cancellation, retry, lease, hibernation, cleanup và circuit breaker không trộn trạng thái giữa hai job.
- [x] Backend test và frontend test/lint/build đạt.
- [x] Deploy additive, canary và rollback gate được ghi bằng bằng chứng thật.
- [x] Không còn runner canary, file fixture hoặc object/job/chunk mồ côi khi đóng.
- [x] Tài liệu ghi trung thực những phép đo chưa thực hiện; không gọi unit test là bằng chứng production.
