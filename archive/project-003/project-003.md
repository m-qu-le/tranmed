...
| Mã kế hoạch | P003 |
| Ngày lập | 15-07-2026 |
| Trạng thái tổng | **HOÀN THÀNH — đóng ngày 16-07-2026 theo quyết định của chủ dự án.** Production dùng B4 gia cố `p003-v3`: context toàn PDF, coverage bắt buộc, strict PASS và tối đa hai repair/reverify |
...
Mỗi job quality `p003-v3` bắt đầu bằng một giai đoạn và sau đó mỗi chunk chạy bốn giai đoạn bắt buộc:
...
Nếu S4 trả `FAIL` ở bất kỳ severity nào, kể cả minor:

```text
S5 REPAIR 1 — sửa có mục tiêu mọi lỗi theo S4, thinking high
    |
    v
S6 REVERIFY 1 — xác minh lại toàn chunk, thinking high
    |
    +-- FAIL + coverage đầy đủ --> S7 REPAIR 2 --> S8 REVERIFY 2
```

- Chỉ `PASS` cùng coverage `COMPLETE` mới được đặt `qualityStatus = passed`; không còn passed-with-minor.
- Cho phép tối đa hai chu kỳ repair/reverify. Vòng hai dùng bản repaired và báo cáo reverify mới nhất.
- Nếu reverify vòng hai vẫn fail, hoặc coverage verify/reverify thiếu, lưu bản tốt nhất, đặt `qualityStatus = needs_review` và hoàn thành job với cảnh báo; không lặp vô hạn.
...
8. Không coi việc model tự tìm được ít lỗi là bằng chứng toàn chunk sạch; mỗi PASS phải có coverage COMPLETE và không có lỗi ở bất kỳ severity nào. `needs_review` vẫn là cổng an toàn cho trường hợp AI không tự xử lý hết.
...
| G8 | Regression, tải, lỗi và benchmark 20 PDF | Hoàn thành | Test tự động, benchmark thật, canary và bằng chứng tài nguyên trong phạm vi được chủ dự án chấp nhận |
| G9 | Rollout production và đóng dự án | Hoàn thành | `p003-v3` đã deploy; các cổng batch/24h/rollback drill được chủ dự án chủ động miễn khi đóng dự án |
...
- [x] **P003-G5-S05 — Repair prompt.** Chỉ sửa lỗi từ report xác minh mới nhất trên bản revised/repaired mới nhất, không quay về draft và không sáng tác thêm giải thích.
...
- [x] **P003-G8-S02 — Unit test stage machine.** PASS trực tiếp, minor/major FAIL→repair, vòng hai dùng artifact mới nhất, FAIL sau repair thứ hai→needs_review, invalid JSON và MAX_TOKENS.
...
- [x] **P003-G8-S12 — Tài nguyên.** Báo cáo local đo RSS split PDF và BSON; canary production ghi nhận Job khoảng 5,5 KB, context khoảng 3,7 KB, 39 chunk khoảng 1,10 MB, source R2 deleted và backlog 0. Chủ dự án chấp nhận bằng chứng này và miễn đo thêm RAM/disk Render để đóng dự án.
...
- [x] **P003-G9-S07 — Rollback drill (được miễn).** Đường rollback `TRANSLATION_PIPELINE_MODE=legacy` đã có config/test và legacy production smoke; chủ dự án quyết định không toggle production lần nữa khi đóng dự án. Không tuyên bố đã thực hiện live rollback drill.
- [x] **P003-G9-S08 — Theo dõi 24 giờ (được miễn).** Chủ dự án miễn cửa sổ theo dõi 24 giờ. Điểm kiểm cuối sau deploy `p003-v3`: health/readiness đạt, Mongo/R2 available, cleanup/upload backlog 0; không trình bày đây là số liệu 24 giờ.
- [x] **P003-G9-S09 — Tối ưu sau dữ liệu.** Chốt không bỏ S3, không thêm micro-glossary hoặc độ phức tạp mới: dữ liệu hiện có không chứng minh lợi ích đủ để thay đổi flow B4 gia cố.
...
- [x] **P003-G9-S11 — Đóng dự án.** Chủ dự án chấp nhận production `p003-v3`, miễn các phép thử bổ sung nêu tại D021 và yêu cầu đóng PROJECT 003 ngày 16-07-2026. Không còn blocker trong phạm vi đã chốt.
...
| 15-07-2026 | E003 | Baseline: backend 44/44 test; frontend 9/9 test, lint/build đạt; npm audit 0; Node 22.17.1, npm 10.9.2, `@google/genai` 1.52.0 | Chi tiết tại `archive/project-003/project-003-baseline.md` |
| 15-07-2026 | E004 | Smoke B0–B4 trên trang mẫu AKI đạt: mọi call `STOP`; B4 audit FAIL 3 lỗi → revise → verify PASS, không cần repair | Metadata tại `archive/project-003/project-003-benchmark-smoke.md`; raw artifact local ignored |
...
| 15-07-2026 | E008 | Benchmark B0–B4 đủ 20 PDF/100 artifact; B4 có 19 PASS, 1 `needs_review`, 4 repair/reverify; 174 attempt, 2 lỗi 429 được chuyển key, 4 schema-invalid được rotate/recover, không có 5xx/auth/non-STOP/empty | `archive/project-003/project-003-benchmark-report.json`, `archive/project-003/project-003-benchmark-review.md` và audit fixture 5/5 |
...
| 16-07-2026 | E011 | Smoke end-to-end quality một PDF 1 trang dùng Mongo/R2 thật nhưng namespace cô lập: 6 stage persisted, PASS sau 1 repair, preview/download khớp, R2 source deleted, Mongo database dropped, tổng 90.270 ms | `archive/project-003/project-003-quality-smoke-report.json`; không cần chạy lại khi quota đang thấp |
| 16-07-2026 | E012 | Case Asthma cho thấy repair cũ co từ khoảng 13,9 KB xuống 5,4 KB dù `STOP`; coverage guard 80% đã rotate output cụt, rerun giữ 13,7 KB và vẫn gắn `needs_review` cho 2 lỗi major | `archive/project-003/project-003-asthma-independent-review.md`, benchmark report cập nhật và commit `8826ad0` |
| 16-07-2026 | E013 | Phân tích không gọi Gemini: B4 trung bình 62.389 ms/chunk, ngoại suy 191 chunk trên 2 lane khoảng 99,3 phút; request tối đa 27,5 giây; PDF lớn nhất làm RSS tăng tối đa 53,87 MiB; BSON terminal ước tính 3,76 MiB/corpus | `archive/project-003/project-003-performance-resource.md`; số liệu Render/Mongo thực vẫn chờ canary |
...
| 16-07-2026 | E015 | Full-corpus live hoàn tất đủ 20 PDF/370 trang/191 chunk: 177 PASS, 14 `needs_review`, 50 repair; 864 call thành công/905 attempt, 0 task failed, stderr rỗng; coverage revise thấp nhất 91,8%, repair 98,4%; 1 critical và 15 major được đưa vào review queue theo file/trang | `archive/project-003/project-003-full-corpus-report.json/.md`; raw artifact từng ở `.p003-local/` và đã được xóa tại D022 |
| 16-07-2026 | E016 | Tạo bộ duyệt local cho đủ 14 chunk `needs_review`: mỗi case có PDF nguồn đúng 2 trang, bản dịch cuối và phiếu chứa finding/evidence/ba lựa chọn; case critical được xếp số 01 | Bundle local 14/14 case, 42/42 file là bằng chứng lịch sử; đã hoàn tất vai trò và được xóa tại D022 |
...
| 16-07-2026 | E023 | Commit `f7a8b07` đã push lên `main` và feature; Render restart lúc `2026-07-16T15:41:03.348Z`, health/readiness đạt, Mongo/R2 available, backlog 0. Job canary terminal `p003-v2` cũ vẫn preview/download trùng hash sau deploy | Xác nhận deploy `p003-v3` không phá dữ liệu terminal cũ; không tạo job dịch mới |
| 16-07-2026 | D021 | Chủ dự án yêu cầu cập nhật hồ sơ và đóng PROJECT 003. Batch nhiều PDF đã bỏ từ D019; phép đo RAM/disk Render bổ sung, live rollback drill và theo dõi 24 giờ được miễn. Chấp nhận bằng chứng hiện có và giữ các giới hạn này trong hồ sơ | G8-S12, G9-S07–S09, G9-S11 và điều kiện hoàn thành cuối được đóng theo owner waiver, không hồi tố tuyên bố các phép thử đã chạy |
| 16-07-2026 | D022 | Sau khi cả P001–P003 đóng, chủ dự án yêu cầu dọn code/file thử nghiệm không còn cần thiết | Xóa `.p003-local` (artifact benchmark/review cục bộ), frontend `dist`, asset template không dùng và harness benchmark một lần; chuyển báo cáo bằng chứng đã lọc sang `archive/project-003/`; giữ `samplepdf`, regression production, migration/backup/reconcile/smoke | Sau cleanup: backend 94/94 test đạt; frontend 12/12 test, lint và build đạt |
...
- [x] Canary đạt; batch 5 PDF và theo dõi production 24 giờ được chủ dự án loại khỏi phạm vi/miễn khi đóng dự án, có ghi rõ tại D019/D021.
...
- Nhánh hiện hành khi dọn hồ sơ: `feature/project-003-translation-quality`.
- Mốc commit đóng P003: `5edce7a`; đã có trên `main` và `feature/project-003-translation-quality` trước lượt dọn cuối.
- Backend/frontend P003 đã deploy; default và effective production mode là `quality`, pipeline/prompt mới là `p003-v3`/`p003-prompts-v3`.
- Canary production dài đã hoàn tất; runner cũ đã dừng. Tại thời điểm dọn cuối không có runner benchmark/canary nào còn chạy.
- Artifact benchmark cục bộ `.p003-local/` đã được xóa sau khi giữ lại báo cáo tổng hợp không chứa nội dung sách. `samplepdf/` là bộ PDF đầu vào của người dùng nên được giữ và vẫn Git ignore.
...
### 13.2. Thành phần được giữ sau khi dọn codebase

- `med-translator-backend/scripts/smoke-project-003-quality.js` và lệnh `npm run smoke:p003-quality`: smoke end-to-end cô lập, cleanup trong `finally`; chỉ chạy khi có quyết định mới vì có dùng dịch vụ thật.
- `archive/project-003/project-003-*.md/json`: báo cáo bằng chứng đã lọc, không chứa raw PDF, prompt/response đầy đủ hoặc secret.
- Harness benchmark P002/P003 và các test chỉ kiểm harness đã được xóa; đây không phải code production hoặc regression của production.
...
- `med-translator-backend/src/services/translationQuality.js`: audit/verify bắt buộc evidence coverage; coverage chưa hoàn chỉnh không thể PASS.
- `med-translator-backend/src/services/qualityPipelineState.js`: strict PASS, mọi severity đều repair, tối đa hai vòng và telemetry riêng cho vòng hai.
- `archive/project-003/project-003.md`: hồ sơ quyết định, giới hạn và bàn giao cuối.

### 13.3. Việc còn lại

Không còn việc bắt buộc trong PROJECT 003. Mọi cải tiến sau này như human-review workflow, memory từ sửa chữa đã duyệt, model khác hoặc giám sát production dài hạn phải được mở thành dự án mới; không tự nối lại benchmark P003.
