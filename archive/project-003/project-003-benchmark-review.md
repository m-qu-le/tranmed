...
- 0 response rỗng, 0 finish reason khác `STOP`, 0 structured report hỏng sau rotation và 0 length anomaly sau khi thêm coverage guard.
- 174 request attempt trong lượt tổng hợp hiện hành; phân bố theo key index là 24–26 attempt/key. Có 2 lượt 429 và 4 JSON sai schema; tất cả được chuyển key/retry hữu hạn, không có 5xx hoặc auth failure.
...
| B4 pipeline high | 88 | 92 | 14.474 ms | 3.958 | 1.798 | 3.245 | 19 passed, 1 needs-review |
...
- 19/20 mẫu đạt `passed`; `78 Asthma.pdf` trang 5–6 còn một omission major và một terminology major theo reverify nên được giữ ở `needs_review`.
...
- Audit đầu ghi nhận 42 finding (15 major, 27 minor). Final verify/reverify còn 10 finding, trong đó 8 minor và 2 major. Minor đơn thuần không kích hoạt vòng repair.
...
- Rà soát Asthma phát hiện bản repair cũ co từ khoảng 13,9 KB xuống 5,4 KB dù `STOP`. Production và benchmark đã thêm coverage guard 80%; rerun giữ 13,7 KB (coverage repair 100,8%) và vẫn cảnh báo hai finding major thay vì âm thầm thay bản đầy đủ bằng bản cụt.
