...
| G9 | Kiểm thử end-to-end và triển khai | Đang làm; batch 5 file lộ lỗi restart/feeder, hotfix đã deploy | Production smoke test đạt, có phương án rollback |
...
- [ ] **P001-G8-S15 — Test frontend.** Đã cover Local Feeder 100 file và retry đúng file `FILE_MISSING`; còn SSE reconnect, partial upload, cancellation và filename collision.
...
- [ ] **P001-G9-S07 — Smoke end-to-end.** Batch 5 PDF production ngày 15-07-2026: 1 completed, 4 `FILE_MISSING`; tìm thấy idle queue loop, feeder advance race và thiếu HTTP keepalive. Ba hotfix đã live; cần chạy lại batch sau `Ctrl+F5`.
...
| T13 | Render restart khi processing | Lease recovery đúng; FILE_MISSING không lặp vô hạn | Incident production đã tái hiện; hotfix retry đúng file + HTTP keepalive đã live, chờ retest |
...
| D006 | 15-07-2026 | Batch thật 5 file có 4 `FILE_MISSING`; Render Free xóa filesystem khi spin-down/restart/deploy, còn feeder dùng capacity trống như tín hiệu đi tiếp | Mất PDF nguồn và bỏ lỡ auto re-upload; log Mongoose spam do idle loop | Dừng idle microtask loop; chỉ advance sau `completed`; re-upload cùng ID; HTTP keepalive 5 phút khi Local Queue active | Hotfix live, chờ retest |
